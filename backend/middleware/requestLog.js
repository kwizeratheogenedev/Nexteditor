...
// unless they fail.
export function requestLogger({ log = defaultLogger, slowMs = 5000 } = {}) {
  return function requestLog(req, res, next) {
    ...
    res.on('finish', () => {
      ...
      const noisy = url.startsWith('/health') || url.startsWith('/clips') || url.includes('/progress/') || url.includes('/status/');
