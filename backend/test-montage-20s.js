...
function createVideo(output) {
  ...
  execFileSync(ffmpegPath, ['-y', '-f', 'lavfi', '-i', 'color=c=black:s=320x240', '-t', '20', '-r', '30', '-c:v', 'libx264', '-pix_fmt', 'yuv420p', output], { stdio: 'ignore' });
}

function createAudio(output) {
  try { if (fs.existsSync(output)) fs.unlinkSync(output); } catch (e) {}
  execFileSync(ffmpegPath, ['-y', '-f', 'lavfi', '-i', 'sine=frequency=440:duration=20', '-q:a', '9', output], { stdio: 'ignore' });
}

async function main() {
  console.log('Creating 20s test media...');
  createVideo(videoA);
  createVideo(videoB);
  createVideo(videoC);
  createAudio(audio);

  console.log('Preparing form data with smoothTransitions=true...');
  ...
  form.append('smoothTransitions', 'true');
